...
${body}`;
  ...
  function initGeometry() {
    ...
    while (ads.length < MaxAds) {
      let spot = rng4(buildings.length);
      ...
      let ad = {ind: ads.length, at, up};
      ads.push(ad);
      let w = 5 + rng4(5), h = 30 + rng4(20);
      let adShape = towerMesh([[-w, -1], [w, -1], [w, 1], [-w, 1]], [[0, 0], [1.2, 0], [1.2, 0.1 * h], [1, 0.15 * h], [1, 0.85 * h], [1.2, 0.9 * h], [1.2, h], [0, h]]);
      ts(adShape, axisRotation([0, 1, 0], spot % cityCols / cityCols * PI2));
      adShape.common = {
        type: [AD, ...ad.at],
        shape: ad.ind,
        up: ad.up
    ...
    while (debris.length < MaxDebris) {
      ...
      if (building && building.height * rng4() < building.density && building.density > 0.4) {
        ...
        let at = lastOf(building.verts).at;
        let up = sub(at, building.verts[building.verts.length - 2].at);
        let d = {ind: debris.length, at, up, score};
        debris.push(d);
      }
    }
    for (let d of debris) {
